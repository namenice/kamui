// src/pages/devices/HardwareTypeManagement.tsx
import { useState, useEffect, useRef } from 'react';
import { useQuery, useMutation, useQueryClient, keepPreviousData } from '@tanstack/react-query';
import { useForm } from 'react-hook-form';
import { toast } from 'react-toastify';
import { hardwareTypeApi, type HardwareType } from '../../api/devices/HardwareTypes';
import { 
  Plus, Edit, Trash2, X, Cpu, // 👈 ใช้ icon Cpu แทน Hardware Type
  Clock, AlertTriangle, ChevronLeft, ChevronRight, 
  Search, ArrowUp, ArrowDown, ArrowUpDown, Tag
} from 'lucide-react';

export default function HardwareTypeManagement() {
  ...
  const { data, isLoading, isError, isPlaceholderData } = useQuery({
    queryKey: ['hardwareTypes', page, debouncedSearch, sortConfig], 
    queryFn: () => hardwareTypeApi.getHardwareTypes(page, limit, debouncedSearch, sortConfig.key, sortConfig.direction),
  ...
  const openModal = (type?: HardwareType) => {
    if (type) {
      setEditingType(type);
      reset({ name: type.name, category: type.category || '', description: type.description || '' });
    } else {
      setEditingType(null);
  ...
  const mutation = useMutation({
    mutationFn: (formData: any) => {
      if (editingType) return hardwareTypeApi.updateHardwareType(editingType.id, formData);
      return hardwareTypeApi.createHardwareType(formData);
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['hardwareTypes'] }); setIsModalOpen(false); toast.success(editingType ? 'Type updated' : 'Type created'); },
    onError: (err: any) => toast.error(err.response?.data?.message || 'Operation failed')
  });

  const deleteMutation = useMutation({
    mutationFn: hardwareTypeApi.deleteHardwareType,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['hardwareTypes'] }); setDeleteModal({ isOpen: false, id: null, name: '' }); toast.success('Type deleted'); },
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
              <Cpu className="w-6 h-6 text-blue-600" /> Hardware Types
            </h1>
            <p className="text-slate-500 text-sm mt-1">Manage equipment categories (Servers, Switches, Storage)</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative group w-full sm:w-64">
                ...
                <input type="text" placeholder="Search type..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="block w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg leading-5 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-shadow shadow-sm" />
            </div>
            <button onClick={() => openModal()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 whitespace-nowrap"><Plus className="w-4 h-4" /> Add Type</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
        <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50 border-b border-gray-200">
                <tr>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 select-none" onClick={() => handleSort('name')}>
                        <div className="flex items-center">Type Name <SortIcon columnKey="name" /></div>
                    </th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Category</th>
                    
                    {/* 👇 แสดงจำนวน Devices */}
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Devices</th>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
                {typesList.map((item: HardwareType) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors duration-150 group/row">
                    ...
                    <td className="px-6 py-4 text-center">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            Number(item.hardwareCount) > 0 ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-500'
                        }`}>
                            {item.hardwareCount || 0}
                        </span>
                    </td>
                    ...
                    <tr>
                        <td colSpan={6} className="text-center py-12 text-gray-500">
                            <div className="flex flex-col items-center justify-center">
                                ...
                                <p>No hardware types found matching "{debouncedSearch}"</p>
                            </div>
                        </td>
                    </tr>
                )}
         ...
         >
             <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                 <div className="bg-slate-900 px-6 py-4 flex justify-between items-center">
                    <h3 className="text-white font-bold text-lg flex items-center gap-2">{editingType ? <Edit className="w-5 h-5"/> : <Plus className="w-5 h-5"/>} {editingType ? 'Edit Type' : 'Create Type'}</h3>
                 ...
                 <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="p-6 space-y-5">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Type Name <span className="text-red-500">*</span></label>
                        <input {...register('name', {required:true})} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Rack Server" />
                        {errors.name && <span className="text-red-500 text-xs">Name is required</span>}
                    </div>

                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Category</label>
                        <input {...register('category')} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Compute, Network, Storage" />
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 mt-2">
                        ...
                        <button type="submit" disabled={mutation.isPending} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-md">{mutation.isPending ? 'Saving...' : 'Save Type'}</button>
                    </div>
                 </form>
             </div>
         </div>
      ...
      {deleteModal.isOpen && (
          ...
          >
              <div className="bg-white rounded-2xl w-full max-w-md p-8 text-center shadow-2xl transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                  ...
                  <h3 className="font-bold text-2xl text-slate-800 mb-3">Delete Type?</h3>
                  <div className="text-slate-500 mb-8 text-base leading-relaxed">Are you sure you want to delete type <br/><span className="font-bold text-slate-900 text-lg">"{deleteModal.name}"</span>?<div className="mt-2 text-sm text-red-500 bg-red-50 py-1 px-2 rounded-lg inline-block">This action cannot be undone.</div></div>
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      <button ref={deleteModalRef} onClick={() => deleteModal.id && deleteMutation.mutate(deleteModal.id)} className="w-full py-3 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700 shadow-lg shadow-red-600/20 flex items-center justify-center gap-2 transition-all active:scale-95"><Trash2 className="w-5 h-5"/> Delete Type</button>
                  </div>
              </div>
          </div>
      )}
