// src/pages/organization/tenancy/TenantManagement.tsx
...
import { tenantGroupApi } from '../../../api/organization/tenancy/tenantGroup';
import { 
  Plus, Edit, Trash2, X, User, Users, // 👈 ใช้ icon User สำหรับ Tenant, Users สำหรับ Group
...
export default function TenantManagement() {
  ...
  const { data, isLoading, isError, isPlaceholderData } = useQuery({
    queryKey: ['tenants', page, debouncedSearch, sortConfig], 
    queryFn: () => tenantApi.getTenants(page, limit, debouncedSearch, sortConfig.key, sortConfig.direction),
    placeholderData: keepPreviousData,
  });

  // 2. Load Tenant Groups (For Dropdown)
  const { data: groupsData } = useQuery({
    queryKey: ['tenantGroups', 'dropdown'],
    queryFn: () => tenantGroupApi.getTenantGroups(1, 100, '', 'name', 'asc'),
    staleTime: 5 * 60 * 1000,
  });

  // --- Form Setup ---
  const { register, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm();
  const selectedGroupId = watch('tenantGroupId');
  ...
  const openModal = (tenant?: Tenant) => {
    if (tenant) {
      ...
      setValue('tenantGroupId', tenant.tenantGroupId); // Set Dropdown value
    } else {
      setEditingTenant(null);
      reset({ name: '', description: '', tenantGroupId: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const mutation = useMutation({
    mutationFn: (formData: any) => {
      if (editingTenant) return tenantApi.updateTenant(editingTenant.id, formData);
      return tenantApi.createTenant(formData);
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['tenants'] }); setIsModalOpen(false); toast.success(editingTenant ? 'Tenant updated' : 'Tenant created'); },
    onError: (err: any) => toast.error(err.response?.data?.message || 'Operation failed')
  });

  const deleteMutation = useMutation({
    mutationFn: tenantApi.deleteTenant,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['tenants'] }); setDeleteModal({ isOpen: false, id: null, name: '' }); toast.success('Tenant deleted'); },
    onError: (err: any) => toast.error(err.response?.data?.message || 'Delete failed')
  });

  const tenantsList = data?.results || [];
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
              <User className="w-6 h-6 text-blue-600" /> Tenant Management
            </h1>
            <p className="text-slate-500 text-sm mt-1">Manage customers or tenants ({data?.totalResults || 0} tenants)</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative group w-full sm:w-64">
                ...
                <input type="text" placeholder="Search tenant..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="block w-full pl-10 pr-3 py-2 border border-slate-300 rounded-lg leading-5 bg-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition-shadow shadow-sm" />
            </div>
            <button onClick={() => openModal()} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 whitespace-nowrap"><Plus className="w-4 h-4" /> Add Tenant</button>
        </div>
      </div>
            ...
            <thead className="bg-slate-50 border-b border-gray-200">
                <tr>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 select-none" onClick={() => handleSort('name')}>
                        <div className="flex items-center">Tenant Name <SortIcon columnKey="name" /></div>
                    </th>
                    ...
                    <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                             ...
                             <span className="text-sm font-medium text-slate-700">
                                 {item.group?.name || <span className="text-red-400 text-xs">Unknown Group</span>}
                             </span>
                        </div>
                    </td>
                    ...
                    <tr>
                        <td colSpan={5} className="text-center py-12 text-gray-500">
                            <div className="flex flex-col items-center justify-center">
                                ...
                                <p>No tenants found matching "{debouncedSearch}"</p>
                            </div>
                        </td>
                    </tr>
                )}
         ...
         >
             <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                 <div className="bg-slate-900 px-6 py-4 flex justify-between items-center">
                    <h3 className="text-white font-bold text-lg flex items-center gap-2">{editingTenant ? <Edit className="w-5 h-5"/> : <Plus className="w-5 h-5"/>} {editingTenant ? 'Edit Tenant' : 'Create Tenant'}</h3>
                 ...
                 <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="p-6 space-y-5">
                    ...
                    {/* 👇 Group Dropdown */}
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Tenant Group <span className="text-red-500">*</span></label>
                        <input type="hidden" {...register('tenantGroupId', { required: true })} />
                        
                        <SearchableSelect 
                           options={groupsData?.results || []}
                           value={selectedGroupId} 
                           onChange={(val) => setValue('tenantGroupId', val, { shouldValidate: true })}
                           placeholder="Search or select group..."
                           error={!!errors.tenantGroupId}
                        />
                        {errors.tenantGroupId && <span className="text-red-500 text-xs">Group is required</span>}
                    </div>

                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Tenant Name <span className="text-red-500">*</span></label>
                        <input {...register('name', {required:true})} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Acme Corp" />
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 mt-2">
                        ...
                        <button type="submit" disabled={mutation.isPending} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-md">{mutation.isPending ? 'Saving...' : 'Save Tenant'}</button>
                    </div>
                 </form>
             </div>
         </div>
      ...
      {deleteModal.isOpen && (
          ...
          >
              <div className="bg-white rounded-2xl w-full max-w-md p-8 text-center shadow-2xl transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                  ...
                  <h3 className="font-bold text-2xl text-slate-800 mb-3">Delete Tenant?</h3>
                  <div className="text-slate-500 mb-8 text-base leading-relaxed">Are you sure you want to delete tenant <br/><span className="font-bold text-slate-900 text-lg">"{deleteModal.name}"</span>?<div className="mt-2 text-sm text-red-500 bg-red-50 py-1 px-2 rounded-lg inline-block">This action cannot be undone.</div></div>
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      <button ref={deleteModalRef} onClick={() => deleteModal.id && deleteMutation.mutate(deleteModal.id)} className="w-full py-3 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700 shadow-lg shadow-red-600/20 flex items-center justify-center gap-2 transition-all active:scale-95"><Trash2 className="w-5 h-5"/> Delete Tenant</button>
                  </div>
              </div>
          </div>
      )}
